...
from forgejo_client.issues import format_issue_list, list_issues
...
class ListIssuesTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        query = str(tool_parameters.get("query") or "").strip() or None
        if not base_url:
        ...
        if not owner or not repo:
            ...
            return
        try:
            issues = list_issues(base_url, owner, repo, state=state, query=query,
            ...
            return
        yield self.create_text_message(format_issue_list(issues, owner, repo, kind="issues"))
        yield self.create_json_message({
            "owner": owner, "repo": repo, "state": state, "count": len(issues),
        })
