...
from forgejo_client.issues import format_issue, get_issue, get_issue_comments
...
class GetIssueTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if not owner or not repo or index is None:
            ...
            return
        try:
            issue = get_issue(base_url, owner, repo, index, token=token)
            comments = get_issue_comments(base_url, owner, repo, index, token=token)
            ...
            return
        yield self.create_text_message(
            format_issue(issue, comments, owner=owner, repo=repo))
